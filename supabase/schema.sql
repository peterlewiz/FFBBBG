...
-- Shady Corner's hand-made weekly power rankings do NOT have their own
-- table. They live in fantasypros_cache above, under the single key
-- `shady-rankings:<league_id>:<season>`, as a jsonb blob of
-- {rows: [{week, user_id, rank, note}]}.
--
-- That looks like the wrong home for them, and in isolation it is - the
-- reason is that nothing in this app can run DDL. The browser only ever
-- holds the anon key, so a dedicated table would have to be created by
-- hand in the Supabase dashboard before the Shady Corner editor worked
-- at all, and the person who writes the rankings isn't the person with
-- dashboard access. Reusing a table that already exists means the page
-- works for whoever has the passphrase, with no setup step.
--
-- If a real table is ever wanted, the migration is a read of that one
-- key and an insert per row; see src/lib/shadyRankings.ts.
